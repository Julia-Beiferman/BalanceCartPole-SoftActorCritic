#!/usr/bin/env python
"""
Aggregate SAC cartpole/swingup logs over seeds and plot mean ± std.

Make sure to run the training script for seeds 0,1,2 before this.
"""
...
def main(args):
    ...
    for seed in seeds:
        ...
        # assume alignment across seeds; if not, you can interpolate
        train_arrays.append(train_returns)
        eval_arrays.append(eval_returns)

    train_arrays = np.array(train_arrays)  # shape: [num_seeds, T_train]
    eval_arrays = np.array(eval_arrays)    # shape: [num_seeds, T_eval]
