"""
Soft Actor-Critic (SAC) for DeepMind Control Suite cartpole/swingup.
CleanRL-style, single-file implementation.

Train with:
    python sac_cartpole_dmc_cleanrl.py --seed 0
    python sac_cartpole_dmc_cleanrl.py --seed 1
    python sac_cartpole_dmc_cleanrl.py --seed 2
"""
...
class DMCCartpoleSwingupEnv:
    """
    Light wrapper around DeepMind Control Suite cartpole/swingup.

    Converts observations to flattened numpy arrays.
...
class SACConfig:
    ...
    update_every: int = 1
    alpha: float = 0.2            # fixed temperature
